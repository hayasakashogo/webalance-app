...
import { useCoupleContext } from "@/app/(app)/app/_components/context/coupleContext/CoupleContext";

const ShareMsgDialog = () => {
    const dialogCookieKey = "dialog_checked";
    const [isOpen, setIsOpen] = useState(false);
    const [dontShowAgain, setDontShowAgain] = useState(false);
    const { couple, partner } = useCoupleContext();
            ...
            <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
                <DialogContent className="w-[90%] max-w-[430px] rounded-md">
                    ...
                    <div className="flex items-center gap-4 justify-center mt-4">
                        <p className='text-text font-bold text-center border border-primary bg-base px-8 py-1 rounded-sm'>{couple.coupleId}</p>
